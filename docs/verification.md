...
- GPS fix is inside the pin radius, or the browser-reported accuracy circle overlaps the pin radius;
...
- GPS center falls outside the pin, but the reported accuracy circle still overlaps the pin radius;
- server ping fails;
- the pin is a recon/cafe/third-space candidate.

## Rejected

A ping is `rejected` when it is outside both the geofence and the reported GPS accuracy circle, lacks a valid contributor session, or sends malformed GPS/timing evidence. Rejected pings remain useful as teaching data, but they do not check off the stop.
